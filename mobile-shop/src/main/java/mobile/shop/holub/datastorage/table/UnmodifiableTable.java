...
/**
 * This decorator of the Table class just wraps another table, but restricts access to methods that don't modify the
 * table. The following methods toss an {@link UnsupportedOperationException} when called:
 * <PRE>
 * public void  insert( String[] columnNames, Object[] values ) public void  insert( Object[] values ) public void
 * update( Selector where ) public void  delete( Selector where ) public void  store ()
 * </PRE>
 * Other methods delegate to the wrapped Table. All methods of the {@link Table} that are declared to return a
...
public class UnmodifiableTable implements Table {
    ...
    public int insert(Collection c, Collection v) {
        illegal();
        return 0;
    }
    ...
    /**
     * Extract the wrapped table. The existence of this method is problematic, since it allows someone to defeat the
     * unmodifiability of the table. On the other hand, the wrapped table came in from outside, so external access is
     * possible through the reference that was passed to the constructor. Use the method with care.
     */
