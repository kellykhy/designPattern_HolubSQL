...
public interface Table extends Serializable, Cloneable {
    /**
     * A convenience constant that makes calls to {@link #commit} and {@link #rollback} more readable when used as an
     * argument to those methods. Use <code>commit(Table.THIS_LEVEL)</code> rather than
     * <code>commit(false)</code>, for example.
     */
    public static final boolean THIS_LEVEL = false;
    /**
     * A convenience constant that makes calls to {@link #commit} and {@link #rollback} more readable when used as an
     * argument to those methods. Use <code>commit(Table.ALL)</code> rather than
    ...
    /**
     * Return the table name that was passed to the constructor (or read from the disk in the case of a table that was
     * loaded from the disk.) THis is a "getter," but it's a harmless one since it's just giving back a piece of
     * information that it was given.
     */
    String name();

    /**
     * Rename the table to the indicated name. This method can also be used for naming the anonymous table that's
     * returned from {@link #select select(...)} or one of its variants.
     */
    void rename(String newName);

    /**
     * Return true if this table has changed since it was created. This status isn't entirely accurate since it's
     * possible for a user to change some object that's in the table without telling the table about the change, so a
     * certain amount of user discipline is required. Returns true if you modify the table using a Table method (like
     * update, insert, etc.). The dirty bit is cleared when you export the table.
     */
    boolean isDirty();

    /**
     * Insert new values into the table corresponding to the specified column names. For example, the value at
     * <code>values[i]</code> is put into the column specified
     * in <code>columnNames[i]</code>. Columns that are not specified are initialized to <code>null</code>.
     *
    ...
    /**
     * In this version of insert, values must have as many elements as there are columns, and the values must be in the
     * order specified when the Table was created.
     *
     * @return the number of rows affected by the operation.
     */
    ...
    /**
     * Update cells in the table. The {@link Selector} object serves as a visitor whose
     * <code>includeInSelect(...)</code> method is called for each row in the table. The return value is ignored, but
     * the Selector can modify cells as it examines them. Its your responsibility not to modify primary-key and other
     * constant fields.
     *
     * @return the number of rows affected by the operation.
     */
     ...
     *
     * @param all if false, commit only the innermost transaction, otherwise commit all transactions at all levels.
     ...
     *
     * @param requestedColumns a collection of String objects representing the desired columns.
     * @param other            a collection of additional Table objects to join to the current one for the purposes of
    ...
    /**
     * Build a representation of the Table using the specified Exporter. Create an object from an {@link Importer} using
     * the constructor with an {@link Importer} argument. The table's "dirty" status is cleared (set false) on an
     * export.
     *
     * @see #isDirty
     */
