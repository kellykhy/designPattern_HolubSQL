 ...
 *    in any of this code.
 */
package mobile.shop.holub.jdbc;
...
import mobile.shop.holub.jdbc.adapters.ConnectionAdapter;
import mobile.shop.holub.sqlengine.Database;
import mobile.shop.holub.sqlengine.text.ParseFailure;

/**
 * A limited version of the Connection class. All methods undocumented base-class overrides throw a {@link SQLException}
 * if called.
 * <p>
 * Note that you can't mix non-autocommit behavior with explicit SQL begin/commit statements. For example, if you turn
 * off autocommit mode (which causes a SQL begin to be issued), and then execute a SQL begin manually, a call to
 * `commit` will commit the inner transaction, but not the outer one. In effect, you can't do nested transactions using
 * the JDBC {@link #commit} or {@link #rollback}  methods.
 *
...
public class JDBCConnection extends ConnectionAdapter {
    ...
    /**
     * Once set true, all SQL statements form a stand-alone transaction. A begin is issued automatically when
     * auto-commit mode is disabled so that the {@link #commit} and {@link #rollback} methods will work correctly.
     * Similarly, a commit is issued automatically when auto-commit mode is enabled.
     * <p>
