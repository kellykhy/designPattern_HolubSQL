...
import mobile.shop.holub.jdbc.adapters.ResultSetAdapter;

/**
 * A limited version of the result-set class. All methods not shown throw a {@link SQLException} if called. Note the the
 * underlying table actually holds nothing but strings, so the numeric accessors (e.g. {@link #getDouble}) are doing
 * string-to-number and number-to-string conversions. These conversions might fail if the underlying String doesn't
 * represent a number.
 *
...
public class JDBCResultSet extends ResultSetAdapter {
    ...
    /**
     * Wrap a result set around a Cursor. The cursor should never have been advanced; just pass this constructor the
     * return value from {@link Table#rows}.
     */
